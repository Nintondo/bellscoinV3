...
"""Class for bellsd node under test"""
...
class TestNode():
    """A class for representing a bellsd node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bellsd, bells_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        ...
        self.bitcoinconf = self.datadir_path / "bells.conf"
        ...
        # This means that starting a bellsd using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
    ...
    PRIV_KEYS = [
            # address , privkey
            AddressKeyPair('mrkC8jM6J1bw9An9uLNYMFhyCgeSQMPGdp', 'cPmfJ5H4oc9LVLsCaBzthjKFTXC8sJuATDASAmiuHkxXzW81S4y2'),
            AddressKeyPair('mn8NtvvLMLx3wcuiBJxSmsgGN2BoYTwuim', 'cRmY14RaEMWKkfXtoPeZNruEJqP49UHCS9f2cSxyrxNZpNYaChE8'),
            AddressKeyPair('mghC5qNzoyWahSzQXJ6q326BXTWe6ktUs3', 'cSEU4zLY5qEz14sNxy4Rb6dQGb9sY8jQgoREACeN5wmsvX2vqBAC'),
            AddressKeyPair('n1QbhqxpvpPnVAeBVHRmhow3qDUrpuHGNM', 'cW7EBCDYp5hP9mZLXCf2vYRxPPkh3cPpvbFXqT2wKAXPhJEF9H1T'),
            AddressKeyPair('mvheVBr4sxnBYKDzBVxQ9jk42s114HHFuh', 'cU2yAX5aXLxAhCuEgzYXBqEzwtM9kKD9XTPWyhu9U5UPeiSWN8dX'),
            AddressKeyPair('mwCvzfkU8YBM5nZtisFdQgZYXzMYc7xeAB', 'cRtFTEHC9zFop8XuPATR3UYTxRd5ADZDHGjmtJP7BzQCXuqZ6jUW'),
            AddressKeyPair('mfh4oKWh8A7UuYX9jyyZLheEe4tPDKBT27', 'cPEsrh2NW4pcN1qn3DUn1gTDCwboPuTbQnyAHs3MZ4gn7NC64KAQ'),
            AddressKeyPair('mgiTE8vZo9ooCxZnfGNNMrPJQg4hRdMRhK', 'cSFZ9Dkp2nsEmi3ttWDaNK8WqVzdqa44DZw5UHWfPuFFC7jdtXh6'),
            AddressKeyPair('ms8y3kb6VeJpjLmwd8h36uBzcgbiT12hPM', 'cU4JTwDj5NMyKVRMbyvAFDVvcoq8kU6aXymqy3HgEuvmxEYvXwts'),
            AddressKeyPair('mwTXu4Z1GYrwPRWV7ZA5xBvEbjuTDEwEWd', 'cVa1go59vtJrBChbQxaZ7PoPazmsd6vgG8pSRKcYX6tvQgyQR4eM'),
            AddressKeyPair('mqJ4yuSkskEL4wCzn1whxcLyv4CXFE6i8Z', 'cTutpBw9e78nStS9jKLqDQwpDNb1R2i1bobmUWVUhGo6nXpxXQQn'),
            AddressKeyPair('mhAy5GsaZk6m2t4PjVsYed5zW14Bb3exiz', 'cPPCDe5VLoCMiJPRTTTCybKaFBrwX8pERp4XLejPLQbmbUpxztrp'),
    ]
    ...
    def __del__(self):
        # Ensure that we don't leave any bellsd processes lying around after
        # the test ends
    ...
    #     else:
    #         assert self.rpc_connected and self.rpc is not None, self._node_msg("Error: no RPC connection")
    #         rpc_method = getattr(RPCOverloadWrapper(self.rpc, descriptors=self.descriptors), name)

    #         def rpc_request_logger(*args, **kwargs):
    #             rpc_request = {
    #                 "jsonrpc": "2.0",
    #                 "id": self.id,
    #                 "method": name,
    ...
    #             }
    #             print(f"Sending RPC request: {rpc_request}")  # Log the RPC request
    #             return rpc_method(*args, **kwargs)

    #         return rpc_request_logger
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # unclean shutdown), it will get overwritten anyway by bellsd, and
        ...
        self.running = True
        self.log.debug("bellsd started, waiting for RPC to come up")
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'bellsd exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; bellsd is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to bellsd after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; bellsd is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into bellsd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to bellsd
        expected_msg: regex that stderr should match when bellsd fails

        Will throw if bellsd starts without an error.
        Will throw if an expected_msg is provided and it does not match bellsd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'bellsd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'bellsd should have exited within {self.rpc_timeout}s '
