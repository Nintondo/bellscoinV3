...
std::shared_ptr<CBlock> MinerTestingSetup::FinalizeBlock(std::shared_ptr<CBlock> pblock)
{
    ...
    while (!CheckProofOfWork(pblock->GetPoWHash(), pblock->nBits, GlobParams().GetConsensus())) {
        ++(pblock->nNonce);
    }
...
// {
    ...
    //     blocks.clear();
    //     BuildChain(GlobParams().GenesisBlock().GetHash(), 100, 15, 10, 500, blocks);
    // }
...
// {
    ...
    // BOOST_REQUIRE(ProcessBlock(std::make_shared<CBlock>(GlobParams().GenesisBlock())));
    // auto last_mined = GoodBlock(GlobParams().GenesisBlock().GetHash());
