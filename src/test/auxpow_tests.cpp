...
{
  const Consensus::Params& params = GlobParams ().GetConsensus ();
...
mineBlock (CBlockHeader& block, bool ok, int nBits = -1)
{
  ...
  if (ok)
    BOOST_CHECK (CheckProofOfWorkTests (block.GetPoWHash (), nBits, GlobParams().GetConsensus()));
  else
    BOOST_CHECK (!CheckProofOfWorkTests (block.GetPoWHash (), nBits, GlobParams().GetConsensus()));
}
...
BOOST_FIXTURE_TEST_CASE (auxpow_pow, BasicTestingSetup)
{
  ...
  BOOST_CHECK (!HasValidProofOfWorkTests({block}, params));
