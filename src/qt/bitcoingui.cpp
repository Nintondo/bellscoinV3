...
void BitcoinGUI::createActions()
{
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Bells Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Bells Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bells: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Bells command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Bells network.", "", count);
    } else {
        //: A substring of the tooltip.
...
{
    int estHeadersLeft = blockDate.secsTo(QDateTime::currentDateTime()) / GlobParams().GetConsensus().PoWTargetSpacing().count();
    if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC)
