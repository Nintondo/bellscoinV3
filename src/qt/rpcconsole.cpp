...
void RPCConsole::setClientModel(ClientModel *model, int bestblock_height, int64_t bestblock_date, double verification_progress)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        ui->networkName->setText(QString::fromStdString(GlobParams().GetChainTypeString()));
...
{
    const ChainType chain = GlobParams().GetChainType();
    if (chain == ChainType::MAIN) return;

    const QString chainType = QString::fromStdString(GlobParams().GetChainTypeString());
